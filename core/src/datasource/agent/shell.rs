...
#[datasource]
async fn shell(script: String, timeout: u32, promise: &mut Promise<'_, BashRow>) -> Result<()> {
...
fn test() {
    ...
    smol::block_on(async {
        let (req, resp) = crate::new_req(crate::Args::new());
        {
            ...
            if let Err(err) = shell("echo 'Hello world'".to_string(), 2, &mut promise).await {
                let _ = req.error(err);
            } else {
                let _ = req.ok();
            }
...
fn test_shell_timeout() {
    ...
    smol::block_on(async {
        ...
        smol::spawn(async move {
            ...
            let mut promise = req.head::<BashRow>().await.unwrap();
            if let Err(err) =
                shell("sleep 10;echo 'Hello world'".to_string(), 2, &mut promise).await
            {
                let _ = req.error(err);
            } else {
                let _ = req.ok();
