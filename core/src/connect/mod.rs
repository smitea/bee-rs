...
pub trait Connection : Send + Sync{
    /// 创建结果集
    async fn new_statement(&self, script: &str) -> Result<Statement>;
}
...
pub async fn new_sqlite_connection(instance: &Instance) -> Result<Box<dyn Connection>> {
    let mut connection = sqlite::SqliteSession::new()?;
    register(instance, &mut connection).await?;
    Ok(Box::new(connection))
}

#[cfg(feature = "lua")]
pub async fn new_lua_connection(instance: &Instance) -> Result<Box<dyn Connection>> {
    let mut connection = lua::LuaSession::new();
    register(instance, &mut connection).await?;
    Ok(Box::new(connection))
}

pub async fn register<T: Configure>(instance: &Instance, configure: &mut T) -> Result<()> {
    // 注册扩展内容
