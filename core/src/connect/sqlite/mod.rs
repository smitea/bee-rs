...
impl crate::Configure for SqliteSession {
    ...
    fn register_source(&mut self, ds: Box<dyn DataSource>) -> crate::Result<()> {
...
impl crate::Connection for SqliteSession {
    async fn new_statement(&self, script: &str) -> crate::Result<Statement> {
        let (request, response) = new_req(Args::new());
        let conn = self.connection.clone();

        let script = script.to_string();
        let _ = smol::spawn(async move {
            let req = request;
            debug!("new sql [{}] use sqlite ...", script);
            if let Err(err) = commit_statement(conn, &script, &req) {
                let _ = req.error(err);
            } else {
                let _ = req.ok();
            }
            debug!("[{}] sqlite is ok", script);
        }).detach();
        Ok(response)
    }
}
    ...
    smol::block_on(async move {
        ...
        let mut promise = match rows.next()? {
            Some(row) => {
                ...
                let mut promise = request.new_commit(cols).await?;
                promise.commit(State::from(new_row)).await?;
                promise
            }
            None => {
                // 获取默认的列结构
                ...
                request.new_commit(get_columns(sql_columns)).await?
            }
        };

        // 循环发送数据
        while let Ok(Some(rs)) = rows.next() {
            promise.commit(State::from(get_row(rs)?)).await?;
        }
        Ok(())
    })
}
