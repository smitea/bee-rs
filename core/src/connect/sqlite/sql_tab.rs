use crate::Value;
use crate::{new_req, Args, Columns, DataSource, DataType, Response, Row};
use rusqlite::ffi;
use rusqlite::vtab::{
...
impl SQLTabCursor<'_> {
    ...
    fn collect(&self, args: Args) -> Result<Response, crate::Error> {
        let data_source: Arc<Box<dyn DataSource>> = self.ds.clone();
        let (mut request, statement) = new_req(args);
        let reg = data_source.get_register();
        let ex: Arc<smol::Executor> = reg.get_state();
        // 执行请求
        let _ = ex
            .spawn(async move {
                debug!("collecting [{}] in sql_tab ...", data_source.name());
                if let Err(err) = data_source.collect(&mut request).await {
                    let _ = request.error(err);
                } else {
                    let _ = request.ok();
                }
                debug!("collected [{}] in sql_tab", data_source.name());
            })
            .detach();
        let resp = smol::block_on(async { statement.wait().await })?;
        Ok(resp)
    }
}
...
unsafe impl VTabCursor for SQLTabCursor<'_> {
    ...
    fn next(&mut self) -> Result<()> {
        ...
        if let Some(reader) = reader {
            match smol::block_on(async move { reader.next_row().await }) {
                Some(rs) => {
                    let row = rs?;
                    self.next = Some(row);
                    self.eof = false;
